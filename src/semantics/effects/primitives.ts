...
function defaultRules(): readonly ModuleEffectRule[] {
  ...
  const rules: ModuleEffectRule[] = [
    ...
    ...rulesForModules(["node:process"], ["hrtime", "uptime"], [effect.clockRead]),
    ...rulesForModules(
      ["node:process"],
      [
        "argv",
    ...
    ...rulesForModules(["node:sqlite"], ["DatabaseSync"], [effect.databaseWrite], ["construct"]),
    ...rulesForModules(
      ["node:sqlite"],
      ["DatabaseSync.exec", "DatabaseSync.run", "exec", "run"],
    ...
    ...rulesForModules(["better-sqlite3"], ["Database"], [effect.databaseWrite], ["construct"]),
    ...rulesForModules(
      ["better-sqlite3"],
      ["Database.prepare", "Database.exec", "Database.run", "prepare", "exec", "run"],
      [effect.databaseWrite],
    ),
    ...rulesForModules(
      ["better-sqlite3"],
