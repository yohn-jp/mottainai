...
export async function dispatchHook(event: HookEvent, options: HookDispatchOptions): Promise<HookDecision> {
  try {
    ...
    if (options.capabilityResolver !== undefined) await withTimeout(Promise.resolve(options.capabilityResolver(event.operation, event)), timeoutMs);
    return decideHook(event, options);
  } catch (error) {
    return failureDecision(event, options, error instanceof Error && error.message === "hook timeout" ? "hook_timeout" : "hook_error");
  }
}
