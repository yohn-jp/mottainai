<p align="center">
  <img src="./docs/design/mockups/readme/mottainai-hero.webp" alt="Mottainai — Coding-Agent Orchestration. Bound the work. Preserve the evidence. Delegate authority." width="100%">
</p>
...
```bash
npx -y mottainai@0.9.3 manager
npx -y mottainai@0.9.3 manager --no-open --port 4318
```
...
```bash
npx -y mottainai@0.9.3
```

Claude Code:

```bash
claude mcp add -s user mottainai -- npx -y mottainai@0.9.3 serve --config /absolute/path/to/mottainai.config.json
```

Codex:

```bash
codex mcp add mottainai -- npx -y mottainai@0.9.3 serve --config /absolute/path/to/mottainai.config.json
```
...
All declared package binaries support the bounded `--version` smoke contract and exit without starting a service: `mottainai` and `mtnai` are CLI aliases, while `mottainai-mcp` reports the package version before opening its MCP stdio transport.
