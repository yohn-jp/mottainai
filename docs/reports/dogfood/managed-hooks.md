...
Dogfood evidence for Issue #126. The deterministic fixture is
`src/e2e/managed-hooks-dogfood.spec.ts`. It creates a temporary Git repository,
sends Claude and Codex payloads through the same `hooks dispatch`, and verifies
client-specific exit and stdout projection. Run:
