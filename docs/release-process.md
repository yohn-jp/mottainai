...
3. Update every `mottainai@<old-version>` reference in `README.md` (npx
   ...
   new version near the existing `docs/releases/*.md` links, keeping older
   links intact.
4. Add `docs/releases/<version>.md`: a short writeup of what changed since the
...
5. Add a `## [<version>] - <date>` entry to `CHANGELOG.md` under `[Unreleased]`
   (which stays empty), linking to the new `docs/releases/<version>.md`
...
- Keep `release_notes` (and other free-text fields) to a single paragraph:
  ...
  Reference files by bare path (e.g. `docs/releases/0.9.1.md`) instead of a
  `[text](path)` link.
...
- `docs/releases/<version>.md` (new file)
