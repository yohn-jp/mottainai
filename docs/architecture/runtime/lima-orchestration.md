...
For the normative chronological execution contract across the selected release,
Route 4 provider establishment, this Lima adapter, guest reconciliation, Route 2,
and the exact Route 1 payload, see
[`route4-route1-operation-book.md`](../../operations/deployment/route4-to-route1.md). Open
connection defects are recorded there against stable operation IDs. This file
remains the Lima-adapter component contract and must not become a competing
end-to-end sequence.
...
| --- | --- | --- |
| Consumer | The standalone `mottainai-init` host bootstrap, for one local Linux/KVM host | Any QEMU/KVM-capable deployment path, e.g. a future Proxmox provider, or a manual `qemu-system-x86_64` boot for evidence work (`docs/architecture/runtime/providers/proxmox`) |
...
| What is verified | The exact appliance digest, via the same [OCI Artifact contract](appliance-oci.md) | The same contract, verified independently by that consumer |
...
1. **Resolve and verify the canonical Appliance** (`appliance.rs`) — fetches
   ...
   [`runtime-appliance-oci.md`](appliance-oci.md) describes, and
...
2. **Render one bounded Lima configuration** (`lima.rs`'s
   ...
   documented/public provider surfaces is added, and canonical guest
   semantics are never touched. The production credential-disk and plain-mode
   completion of this step is tracked by #840/#841.
3. **Create/start or reconcile the instance** through exactly the
   documented, public `limactl` operations: `list --all-fields --format
   json`, `create`, `start`, and `shell` (guest exec), plus the public disk
   surface required for the #840 credential carrier. No private state
   files, sockets, or driver internals are product authorities.
4. **Wait for the canonical guest/bootstrap health boundary**, not merely
   ...
   [`linux-runtime-contract.md`](../../contracts/runtime/linux-runtime.md#healthcapability-result).
...
5. **Converge the intended managed generation, when requested** (Issue
   ...
   --descriptor PATH` derives the current manifest/generation projection
   directly from a release deployment descriptor (`docs/contracts/deployment/descriptor`,
   ...
   published sha256 sidecar — no Node or repository checkout is required on
   the host. The missing live projection/consumption of the descriptor-bound
   exact Route 1 payload is tracked by #850; until it closes, this step is not
   the complete Route 3 → Route 2 → Route 1 release handoff described by
   ADR-0003. `--spec PATH` still accepts an already-built specification
   document for diagnostics/advanced use:
...
6. **Report deterministic bounded evidence** — instance name, appliance
   ...
   document (`--json`) or a short human summary. The operation book defines
   the stronger final evidence set required for complete Route 4 → Route 1
   certification, including provider/data/credential and exact Route 1
   payload identities.
...
- An existing instance whose recorded provenance **disagrees** with the
  ...
  mounts, and—after #840—the managed credential identity) fails closed as
  `lima_instance_incompatible`. Recreation is never the automatic
  reconciliation mechanism; an operator must resolve the drift explicitly.
...
[`host-bootstrap.md`](../../contracts/deployment/host-bootstrap.md) already establishes for Route 4.
...
- This does not build, activate, health-check, or roll back a managed
  generation itself. `converge_managed_generation` transports desired state
...
- Hermetic CI exercises the deterministic reconciliation/configuration
  surfaces and separately proves the canonical Appliance artifact bytes.
  The current trusted-main pipeline does **not** yet compose the exact
  production `SystemLimaCli` → rendered Lima configuration → real canonical
  Appliance → SSH → guest-health path; #844 owns that missing canonical
  production-Lima integration proof. Therefore mocked Lima/OCI tests and
  `host-bootstrap/tests/appliance_real.rs` must not be described as evidence
  that the full production provider boundary has executed.
- Real Linux/KVM support certification remains #261 after repository CI has
  proven all deterministic software boundaries.
