...
- Configuration-sensitive inputs are versioned as research fixtures: the
  ...
  [`scripts/fixtures/quality-evidence-providers/v1`](../../../scripts/fixtures/quality-evidence-providers/v1).
...
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| dependency-cruiser | [`dependency-cruiser.no-circular.json`](../../../scripts/fixtures/quality-evidence-providers/v1/dependency-cruiser.no-circular.json) (`fixture-v1`) | `e53af1f3476d0cdd2aebce6e13f44f8556735fe181155ce70d7c42e3b69c26b5` | cwd `repo root`; `pnpm dlx dependency-cruiser@18.1.1 --config scripts/fixtures/quality-evidence-providers/v1/dependency-cruiser.no-circular.json --output-type json src`; resolver `tsconfig.json`; source revision and Node/pnpm versions above |
| Semgrep            | [`semgrep.custom-rules.yml`](../../../scripts/fixtures/quality-evidence-providers/v1/semgrep.custom-rules.yml) (`fixture-v1`)                       | `f9e4aabb9ce137496fa58149f9655c190c19ed7ab5435e0e5d3b24ab64e41d41` | cwd `repo root`; `semgrep@1.172.0 scan --config scripts/fixtures/quality-evidence-providers/v1/semgrep.custom-rules.yml --json --metrics=off --include '*.ts' --exclude '*.d.ts' src scripts`; source revision and Node/pnpm versions above      |
...
| Architecture and dependency direction       | `scripts/architecture-check.mjs`, `docs/governance/coding-standards`, and the TypeScript-resolved architecture checks                                                                 |
| Governance and PR/Issue contract            | canonical `yohn-jp/.github` `pr-governance.yml`, `scripts/product-pr-checks-lib.mjs`, and `docs/governance/issues-and-pull-requests`                                                                                       |
| Repository meaning and dependency relations | Repository Semantic IR and its `imports`, `depends_on`, `uses_package`, and `imports_api` relations; see [`docs/architecture/repository/semantic-model`](../../architecture/repository/semantic-model.md)            |
| Bounded evidence retention and projection   | `ArtifactStore`, `result_id`, Context Runtime projection/budget, and read-evidence state; see [`docs/contracts/runtime/read-governor`](../runtime/read-governor.md) and [`docs/testing/README.md`](../../testing/README.md) |
| Agent/client operation policy               | Managed Hooks dispatcher and operation-specific fail-open/fail-closed policy; see [`docs/architecture/integrations/managed-hooks`](../../architecture/integrations/managed-hooks.md)                                                 |
